...
const KotaniToken = artifacts.require('KotaniToken');
const Minter = artifacts.require('Minter');
const MockERC20 = artifacts.require('MockERC20');

contract('Minter', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.sushi = await KotaniToken.new({ from: alice });
    });

    it('should set correct state variables', async () => {
        this.chef = await Minter.new(this.sushi.address, dev, '1000', '0', '1000', { from: alice });
    ...
    context('With ERC/LP token added to the field', () => {
        ...
        it('should allow emergency withdraw', async () => {
            ...
            this.chef = await Minter.new(this.sushi.address, dev, '100', '100', '1000', { from: alice });
        ...
        it('should not distribute SUSHIs if no one deposit', async () => {
            ...
            this.chef = await Minter.new(this.sushi.address, dev, '100', '200', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            await this.lp.approve(this.chef.address, '1000', { from: bob });
        ...
        it('should distribute SUSHIs properly for each staker', async () => {
            ...
            this.chef = await Minter.new(this.sushi.address, dev, '100', '300', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.chef.add('100', this.lp.address, true);
            ...
            //   Minter should have the remaining: 10000 - 5666 = 4334
        ...
        it('should give proper SUSHIs allocation to each pool', async () => {
            ...
            this.chef = await Minter.new(this.sushi.address, dev, '100', '400', '1000', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
        ...
        it('should stop giving bonus SUSHIs after the bonus period ends', async () => {
            ...
            this.chef = await Minter.new(this.sushi.address, dev, '100', '500', '600', { from: alice });
            await this.sushi.transferOwnership(this.chef.address, { from: alice });
            await this.lp.approve(this.chef.address, '1000', { from: alice });
