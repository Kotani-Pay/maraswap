...
const InvestorsVault = artifacts.require('InvestorsVault');

contract('InvestorsVault', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.sushi = await KotaniToken.new({ from: alice });
        this.bar = await InvestorsVault.new(this.sushi.address, { from: alice });
    ...
    it('should work with more than one participant', async () => {
        ...
        // InvestorsVault get 20 more SUSHIs from an external source.
